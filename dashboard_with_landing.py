...
def _uc2_render_quality_origin(df: pd.DataFrame) -> None:
    ...
    Args:
        df (pd.DataFrame): Session-level dataframe.
    """
    # FIX 1: removed inline import — mticker now imported at top of file
...
def _uc2_plot_yield_box(title_df: pd.DataFrame) -> None:
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    data_orig = title_df[
        title_df["origin_label"] == "Netflix Original"
    ]["total_sessions"]
...
def show_content_investment(_artifacts):
    """Render Use Case 2: Content Investment Optimization dashboard for Sarah."""
    # FIX 2: added docstring
    # FIX 3: renamed artifacts -> _artifacts to suppress unused-argument warning
    ...
    **Action:** Analyze content quality vs. retention impact
    ...
    try:
        ...
        # FIX 4: renamed e -> err and added pylint disable comment
        st.error(f"""
