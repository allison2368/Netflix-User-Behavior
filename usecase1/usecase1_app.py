...
def run_usecase_1():
    ...
    st.markdown("**Marcus' question:** Who are the high-risk churners and what marketing methods can he implement to draw them back?")
    st.divider()
    ...
    st.markdown(
        "We used K-means clustering using **Recency**, **Frequency**, and **Engagement** "
        "segmentation to create interpretable segments."
    )
    st.markdown(
        "- **Segment 0 — Browsing users:** Users that log in relatively frequently but just browse.\n"
        "- **Segment 1 — Regular viewers:** Most active; they have the highest average total sessions and relatively high completion rate.\n"
        "- **Segment 2 — Dormant users:** Haven't watched in a while, likely churned.\n"
        "- **Segment 3 — Quality viewers:** Less frequent watchers but when they do, they spend a longer time watching."
    )
    ...
    st.markdown(
        ...
        "- **Segment 0:** Send personalized recommendations, make it easier to find content, improve recommendations.\n"
        "- **Segment 1:** Offer premium features, recommend new movies.\n"
        "- **Segment 2:** Send promotions (cheaper deals), send targeted messages.\n"
        "- **Segment 3:** Personalized recommendations, notify of similar content."
    )
    if len(filtered_df) > 0:
