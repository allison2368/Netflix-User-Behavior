"""
Logic for Netflix Churn Prediction - Use Case 1.
Handles data loading from BigQuery, model prediction, and plot generation.
"""
...
def predict_churn_logic(dataframe, artifacts):
    ...
    df["churn_probability_pct"] = (
        artifacts["rf_model"].predict_proba(features_scaled)[:, 0] * 100
    ).round(1)
    return df
...
def get_churn_dist_plot(filtered_data):
    ...
    fig = go.Figure(
        go.Histogram(
            x=filtered_data["churn_probability_pct"], nbinsx=20, marker_color="#E50914"
        )
    )
    fig.update_layout(
...
def get_segment_pie_plot(filtered_data):
    ...
    fig.update_layout(
        title="Distribution by Segment", template="plotly_dark", height=400
    )
    return fig
